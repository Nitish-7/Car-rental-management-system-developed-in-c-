...
public:
  homescreen()
  {
    ...
    cout << "\t\t\t\t\tA) Rent Car\n"
         << "\t\t\t\t\tB) Handle Cars\n"
         << "\t\t\t\t\tC) Customer details\n"
         << "\t\t\t\t\tD) Get Back Car from Customer\n"
         << "\t\t\t\t\tE) Defaulter Customers\n"
         << "\t\t\t\t\tX) to exit\n";
    ...
    else if (homeScreenChoice == 'd' || homeScreenChoice == 'D')
    {
      system("CLS");

      cout << "You have choosed\n\n D) Take Back Car from Customer" << endl;
      CarReturn take;
      take.takeBackCar();
